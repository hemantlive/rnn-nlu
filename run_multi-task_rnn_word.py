...
import word_data_utils as data_utils
  ...
  with tf.gfile.GFile(source_path, mode="r") as source_file:
    with tf.gfile.GFile(target_path, mode="r") as target_file:
      with tf.gfile.GFile(label_path, mode="r") as label_file:
        ...
        source, label = source_file.readline(),label_file.readline()
        counter = 0
...
#          target_ids.append(data_utils.EOS_ID)
          for bucket_id, (source_size, target_size) in enumerate(_buckets):
              #print("source sz %d " % source_size)
              #print("tgt sz %d " % target_size)
              #print("source id len %d" % len(source_ids))
          ...
          #source, target, label = source_file.readline(), target_file.readline(), label_file.readline()
          source, target, label = l, l, label_file.readline()
...
def load_embeddings(data_path):
    with open(data_path + "twitter_embedding_map.pickle", "rb") as fp:
...
def train():
  ...
  #data_utils.prepare_edu_embeddings(FLAGS.data_dir, vocab, len(vocab), FLAGS.word_embedding_size)
  #print(len(vocab))
  #print(len(tag_vocab))
  ...
  with tf.Session() as sess:
    ...
    model, model_test = create_model(sess, len(vocab), 4, len(label_vocab))
    #model, model_test = create_model(sess, len(vocab), len(tag_vocab), len(label_vocab))
    print ("Creating model with source_vocab_size=%d and label_vocab_size=%d." % (len(vocab), len(label_vocab)))
    ...
    '''
    '''
    edu_embed = load_embeddings("./data/twitter/")
      ...
      if current_step % FLAGS.steps_per_checkpoint == 0:
        ...
        # Run evals on development/test set and print the accuracy.
            ...
            if task['tagging'] == 1:
              ...
              precision, recall , f1score , _ = metrics.precision_recall_fscore_support(np.array(gnd_tag), np.array(pred_tag), average='macro')
